...
|:-----|---------:|----------:|-------------:|-------------:|-------------:|-------------:|
| 0001 |    3.381 |     1.826 |        3.381 |        3.381 |        1.826 |        1.826 |
| 0002 |    3.366 |     1.798 |        3.366 |        3.366 |        1.798 |        1.798 |
| 0003 |    3.422 |     1.810 |        3.422 |        3.422 |        1.810 |        1.810 |
| 0004 |    3.427 |     1.812 |        3.427 |        3.427 |        1.812 |        1.812 |
| 0005 |    3.392 |     1.829 |        3.392 |        3.392 |        1.829 |        1.829 |
| 0006 |    3.389 |     1.840 |        3.389 |        3.389 |        1.840 |        1.840 |
...
The above plots show that the distribution of $\hat{r}^2$ estimates are
highest around 1.82, while the highest distribution of
$log(\hat{\beta_0}*\hat{\beta_1})$ sits around 3.40.
...
victim age, sex and race as predictors. We will save the output as an R
...
| Term           | Parameter estimate | Standard error |    OR |   CIL |   CIU |
|:---------------|-------------------:|---------------:|------:|------:|------:|
| victim_sexMale |             -0.854 |          0.138 | 0.426 | 0.325 | 0.558 |

The result of our code shows that in Baltimore, the odds of resolving a
homicide when the victim is male is 0.426 times the odds of resolving a
homicide when the victim is female. We are 95% confident that the true
odds ratio of resolving a homicides lies between 0.325 and 0.558 for
male vs. female victims.

#### For all cities

Now, using `purrr::map`, list columns, and `unnest`, we will run `glm`
for all cities in the homicide dataset, and extract the adjusted odds
...
| city_state      | term           |     log_OR |        OR |       CIL |       CIU |   p.value |
|:----------------|:---------------|-----------:|----------:|----------:|----------:|----------:|
| Albuquerque, NM | victim_sexMale |  0.5695658 | 1.7674995 | 0.8306581 | 3.7609388 | 0.1392989 |
| Atlanta, GA     | victim_sexMale |  0.0000771 | 1.0000771 | 0.6836012 | 1.4630669 | 0.9996829 |
| Baltimore, MD   | victim_sexMale | -0.8544628 | 0.4255117 | 0.3245590 | 0.5578655 | 0.0000000 |
| Baton Rouge, LA | victim_sexMale | -0.9638036 | 0.3814393 | 0.2092532 | 0.6953103 | 0.0016535 |
| Birmingham, AL  | victim_sexMale | -0.1392445 | 0.8700153 | 0.5742951 | 1.3180098 | 0.5111455 |
| Boston, MA      | victim_sexMale | -0.3945382 | 0.6739912 | 0.3560002 | 1.2760222 | 0.2256969 |
...
We will convert `malform` to a logical variable, and convert categorical
variables `babysex`, `frace`, and `mrace` into factors, while recoding
them.
...
```
    ...
    ## 
    ## Call:
    ## lm(formula = bwt ~ bhead + blength + delwt + mrace + frace + 
    ##     smoken + fincome, data = birthweight_df)
    ## 
    ## Residuals:
    ##      Min       1Q   Median       3Q      Max 
    ## -1120.76  -181.39    -3.01   174.48  2397.93 
    ## 
    ## Coefficients:
    ##                     Estimate Std. Error t value Pr(>|t|)    
    ## (Intercept)       -5609.7216    96.6166 -58.062  < 2e-16 ***
    ## bhead               135.3971     3.3895  39.945  < 2e-16 ***
    ## blength              78.5155     2.0170  38.927  < 2e-16 ***
    ## delwt                 2.3215     0.1984  11.703  < 2e-16 ***
    ## mraceblack         -154.9385    46.6890  -3.319 0.000912 ***
    ## mraceasian          -86.8643    72.8330  -1.193 0.233071    
    ## mracepuerto rican   -67.1069    45.6563  -1.470 0.141681    
    ## fraceblack            7.6617    46.8029   0.164 0.869974    
    ## fraceasian           21.7589    70.2788   0.310 0.756874    
    ## fracepuerto rican   -56.1262    45.3053  -1.239 0.215470    
    ## fraceother           -1.3386    75.1282  -0.018 0.985786    
    ## smoken               -4.6517     0.5939  -7.832 5.98e-15 ***
    ## fincome               0.3490     0.1769   1.972 0.048640 *  
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
    ## Residual standard error: 276.5 on 4329 degrees of freedom
    ## Multiple R-squared:  0.7093, Adjusted R-squared:  0.7085 
    ## F-statistic: 880.3 on 12 and 4329 DF,  p-value: < 2.2e-16
...
explain 70.93% of the variability in birthweight. However, we can see
...
```
    ...
    ## mraceblack         -147.3690     9.2591 -15.916  < 2e-16 ***
    ## mraceasian          -74.8522    42.5481  -1.759   0.0786 .  
    ## mracepuerto rican  -117.7264    18.7867  -6.266 4.05e-10 ***
    ## ---
    ## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    ## 
